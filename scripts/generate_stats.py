...
def head(w, h, font=None):
    return (f'<svg xmlns="http://www.w3.org/2000/svg" width="{w}" height="{h}" '
            f'viewBox="0 0 {w} {h}" fill="none" font-family="{MONO}">' + style(font=font))
...
def label(x, y, text, size=11, cls="m-f", anchor="start", extra=""):
    ...
    return (f'<text x="{x}" y="{y}" class="{cls}" font-size="{size}"{a}{extra}>'
            f'{xml_text(str(text))}</text>')
...
def draw_langs(s):
    ...
    for gi, (gx, title, data, as_pct) in enumerate(groups):
        p.append(f'<g opacity="0">{fade(0.10 + gi * 0.10)}' +
                 label(gx, 12, title.upper(), 9, "m-f", extra=' letter-spacing="1.3"') + '</g>')
        if not data:
            continue
        ...
        for ri, (name, val) in enumerate(data):
            ...
            p.append(f'<g opacity="0">{fade(0.24 + gi * 0.10 + ri * 0.05)}' +
                     ...
                     label(gx + colw - 6, y + 8, shown, 11, "m-f", "end") + '</g>')
...
def draw_heading(word):
    ...
    p.append(f'<line x1="{text_end:.0f}" y1="12.5" x2="{WIDTH}" y2="12.5" class="u-s" stroke-width="1"/>')
    p.append("</svg>")
    return "".join(p)
...
def main():
    ...
    for word in ("stack", "projects", "stats"):
